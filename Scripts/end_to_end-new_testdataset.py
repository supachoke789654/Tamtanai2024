...
parser.add_argument('--new_testdataset', default=1, type=int, help='New test dataset version')
args = parser.parse_args()
n_documents = args.n_documents
new_testdataset = args.new_testdataset

INFERENCE_RESULT_PATH = f'/project/lt200301-edubot/Capstone-TamTanai/inference_result/inference_end_to_end_n_documents={n_documents}_new_testdataset_{new_testdataset}.csv'
...
df = pd.read_csv(f'../asset/dataset/new_testdataset_{new_testdataset}.csv')
...
result_df = result_df[['question', 'answer', 'processing_time', 'domain_detection_result', 'domain_detection_probability',
                       'domain_detection_time_usage', 'retrieval_result', 'retrieval_source', 'retrieval_reranking_score',
                       'retrieval_time_usage', 'generation_result', 'generation_time_usage']]
