...
EXAM_DATASET_PATH = "../asset/dataset/exam_training_data.csv"
THAI_LEGAL_DATASET_PATH = "../asset/dataset/thai_legal.csv"
FINETUNED_MODEL_NAME = "typhoon2-qwen2.5-7b-instruct-finetune-dataset-v1-exam-thailegal-with-v1+exam+thailegal-validation-set"
...
def generate_exam_training_prompt(
...
def process_exam_dataset(data: pd.DataFrame):
    data["text"] = data.apply(
        lambda row: generate_exam_training_prompt(
...
exam_data = pd.read_csv(EXAM_DATASET_PATH)
exam_data = process_exam_dataset(exam_data)
law_dataset = pd.concat((law_dataset,thai_legal_data, exam_data))

print("all data =",len(law_dataset))
# train_ratio = 0.8`
...
print("Train set shape:", train_data.shape[0] + exam_data.shape[0])
