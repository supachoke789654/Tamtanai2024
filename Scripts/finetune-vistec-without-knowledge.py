...
MODEL_NAME_OR_PATH = "/project/lt200301-edubot/Capstone-TamTanai/models/Llama-3-Typhoon-1.5-8B"
DATASET_PATH = "../asset/dataset/dataset.csv"
VISTEC_DATASET_PATH = "../asset/dataset/WangchanThaiInstruct_7.24.parquet"
FINETUNED_MODEL_NAME = "Llama-3-Typhoon-1.5-8B-finetune-vistec-dataset-without-knowledge-no-validation-set"
...
DEFAULT_SYSTEM_PROMPT = """คุณคือนักกฎหมายที่จะตอบคำถามเกี่ยวกับกฎหมาย จงตอบคำถามต่อไปนี้
...
{system_prompt.strip()}</s><|im_start|>user
...
def generate_vistec_training_prompt(
    input: str, question: str, response: str,
    system_prompt: str = DEFAULT_SYSTEM_PROMPT
) -> str:
    if not pd.isna(input):
        return f"""<s><|im_start|>system
{system_prompt.strip()}</s><|im_start|>user
{question.strip()}\n{input.strip()}</s><|im_start|>assistant
{response.strip()}</s>
"""
    else:
        return f"""<s><|im_start|>system
...
def process_vistec_dataset(data: pd.DataFrame):
    data["text"] = data.apply(
        lambda row: generate_vistec_training_prompt(
            row["Input"], row["Instruction"], row["Output"]
        ), axis=1
    )
    return data
...
vistec_data = pd.read_parquet(VISTEC_DATASET_PATH)
vistec_data = vistec_data[vistec_data["Domain"] == "Legal"].reset_index(drop=True)

#train_ratio = 0.8
#train_data, validation_data = train_test_split(vistec_data, test_size=1 - train_ratio, random_state=42)

#print("Train set shape:", train_data.shape[0])
...
train_data = process_vistec_dataset(vistec_data)
#validation_data = process_vistec_dataset(validation_data)
#vistec_data = process_vistec_dataset(vistec_data)
#train_data = pd.concat((train_data, vistec_data))
train_dataset = Dataset.from_pandas(train_data)
#validation_dataset = Dataset.from_pandas(validation_data)
