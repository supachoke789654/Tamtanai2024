...
def rerank(question, documents):
    ...
    if 'bge-reranker-v2-m3' in RERANKING_MODEL:
        scores = reranker.compute_score(pairs, normalize=True)
    elif 'gte-multilingual-reranker-base' in RERANKING_MODEL:
        with torch.no_grad():
            inputs = reranking_tokenizer(pairs, padding=True, truncation=True, return_tensors='pt', max_length=512)
            scores = reranker(**inputs, return_dict=True).logits.view(-1, ).float().numpy()
...
if 'bge-reranker-v2-m3' in RERANKING_MODEL:
    ...
    reranking_tokenizer = AutoTokenizer.from_pretrained(RERANKING_MODEL)
    reranker = AutoModelForSequenceClassification.from_pretrained(RERANKING_MODEL, trust_remote_code=True, torch_dtype=torch.float16)
    reranker.eval()
...
start = time.time()
for _, row in tqdm(df.iloc[:20].iterrows(), total=20):
    hit = False
    ...
    retrieved_document = rerank(row['question'], retrieved_documents)
    if '/'.join(row['source'].split('/')[1:]) in retrieved_document.metadata['source']:
        reranked_hit = True
...
with open(f'result_{EMBEDDING_MODEL.split("/")[-1]}_{RERANKING_MODEL.split("/")[-1]}.json', 'w') as file:
